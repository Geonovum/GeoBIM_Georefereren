...
![IfcGref demo](media/ifcgeorefPic.png "IfcGeoRefChecker and IfcGeoRefUpdater User interface")


## IfcGeoRefChecker (Dutch)

IfcGeoRefChecker is een gratis, zelfstandig Windows-gebaseerd softwareprogramma, ontwikkeld door Christian Clemen en Hendrik Görne, bedoeld om de kwaliteit van georeferentie in Building Information Models (BIM) die in IFC-formaat zijn opgeslagen te evalueren en te verbeteren. Het werd geïntroduceerd in hun publicatie uit 2019, “Level of Georeferencing (LoGeoRef) using IFC for BIM”, waarin zij de zogenoemde LoGeoRef-metriek voorstelden, een gestructureerde schaal die aangeeft hoe nauwkeurig een BIM-model is georeferentieerd, variërend van eenvoudige postadresinformatie (LoGeoRef10) tot volledige metadata van een coördinatenreferentiesysteem (LoGeoRef50). De tool operationaliseert dit concept door gebruikers in staat te stellen te controleren in hoeverre hun IFC-modellen voldoen aan de verschillende LoGeoRef-niveaus.

Technisch gezien functioneert IfcGeoRefChecker als een zelfstandige desktopapplicatie voor Windows, die georeferentiegegevens kan extraheren en analyseren uit IFC2x3-, IFC4- en IFC4.1-bestanden. De software controleert parameters zoals geografische coördinaten, translatiewaarden, rotatie ten opzichte van ware noorden, hoogte en gebruikte coördinatenreferentiesystemen om te verifiëren of de ruimtelijke positionering en oriëntatie van het model correct zijn gedefinieerd. Gebruikers kunnen bovendien verschillende IFC-bestanden met elkaar vergelijken en foutieve parameters rechtstreeks aanpassen, ondersteund door een gedetailleerd protocol dat ontbrekende of inconsistente informatie weergeeft. Hierdoor fungeert de software als een instrument voor kwaliteitsborging ter bevordering van de ruimtelijke interoperabiliteit tussen BIM- en GIS-omgevingen.

De software wordt vergezeld door IfcGeoRefUpdater, een aanvullende toepassing waarmee correcties op IFC-bestanden kunnen worden toegepast op basis van de resultaten van de Checker. Samen vormen zij een praktische oplossing voor het verbeteren van de geospatiale consistentie binnen digitale bouwprocessen.
